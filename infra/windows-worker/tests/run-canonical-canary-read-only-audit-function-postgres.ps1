...
function Invoke-Psql {
  ...
  $output = & $psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -p $port -U $User -d postgres -At -c $Sql 2>&1
...
function Invoke-PsqlFile {
  ...
  $savedErrorActionPreference = $ErrorActionPreference
  $ErrorActionPreference = 'Continue'
  $output = & $psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -p $port -U postgres -d postgres -f $Path 2>&1
  $exitCode = $LASTEXITCODE
  $ErrorActionPreference = $savedErrorActionPreference
  if ($exitCode -ne 0) { throw ($output -join "`n") }
}
...
'@ | Out-Null
  ...
  Invoke-PsqlFile (Join-Path $repoRoot 'infra\windows-worker\tests\fixtures\production-hermes-jobs-schema.sql')
  Invoke-Psql @'
...
'@ | Out-Null
  Invoke-PsqlFile (Join-Path $repoRoot 'supabase\migrations\202608030001_canonical_attempt_lease_foundation.sql')
  Invoke-PsqlFile (Join-Path $repoRoot 'supabase\migrations\202608110001_canonical_canary_admission_control.sql')
  Invoke-Psql @'
...
'@ | Out-Null
  Invoke-PsqlFile (Join-Path $repoRoot 'supabase\migrations\202608130001_canonical_canary_read_only_audit_function.sql')
...
values ($(Quote-Sql $policy),$(Quote-Sql $owner),$(Quote-Sql $batch),'worker_read_only',true);
...
values ('$jobId','hermes_canonical_orchestration','sensitive request text','running',
...
values ('$admissionId',$(Quote-Sql $policy),$(Quote-Sql $owner),$(Quote-Sql $batch),
...
"@ | Out-Null
  ...
  Invoke-Psql @"
drop index public.hermes_jobs_source_event_id_unique;
insert into public.hermes_jobs
...
values ('44444444-4444-4444-8444-444444444444','hermes_canonical_orchestration',
  'second sensitive request','queued',$(Quote-Sql $event),'second-message',$(Quote-Sql $owner),
