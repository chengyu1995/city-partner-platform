...
Admission audit records retain only pseudonymous correlation fields. Owner, batch, event, request, and mode candidates are hashed before logging, including DENY decisions where no admission object exists. Persistence denials use the same correlation contract. Raw Feishu owner, event, message, and payload values are never emitted by the Canary admission logger.
