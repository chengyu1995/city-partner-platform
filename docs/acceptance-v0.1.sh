...
check "2.1 category input" "grep -q 'category' /tmp/acc_post.html"
check "2.2 city input" "grep -q 'city' /tmp/acc_post.html"
check "2.3 title input" "grep -q 'title' /tmp/acc_post.html"
check "2.4 description" "grep -q 'description' /tmp/acc_post.html"
check "2.5 contact" "grep -q 'contact' /tmp/acc_post.html"
...
sleep 1
check "2.7 新帖进列表" "curl -sSL '$URL/partners' | grep -q 'acceptance-v0.1-test'"
...
if [ -n "$ID" ]; then
  ...
  check "4.1 标题 (h1)" "grep -q '<h1' /tmp/acc_detail.html"
  ...
  check "4.5 发布时间" "grep -qE '[0-9]{4}|[0-9]+ (秒|分钟|小时|天|周|月)前|ago' /tmp/acc_detail.html"
else
  echo "⚠️ 跳过详情页测试 (无数据)"
fi
...
check "5.1 待审核 tab" "grep -q '待审核' /tmp/acc_admin.html"
...
if [ -n "$ID" ]; then
  check "6.1 详情页有举报按钮" "grep -q '举报' /tmp/acc_detail.html"
